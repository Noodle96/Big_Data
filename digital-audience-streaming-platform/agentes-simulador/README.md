...
- **`--ignore-horario`**: solo `comprador_compulsivo` está activo las 24 horas; los otros 7 perfiles tienen ventanas horarias (`AgentProfile.hours`), así que qué perfiles aparecen en una corrida depende de la hora real del servidor (UTC en las instancias EC2). Este flag ignora esa restricción para que los 8 perfiles estén siempre activos — útil para capturar evidencia con variedad de perfiles sin depender de a qué hora se corre.
